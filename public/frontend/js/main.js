...
function loadContactForm(t) {
    ...
    e.length &&
        (e.show(),
        $.ajax({ url: n, data: { contact_id: t } }).done(function (t) {
            e.html(t),
                e.find(".ajax-content-control").trigger("change"),
                e.find(".contact_form").validate({
                    errorPlacement: function (t, e) {
                        var n = $(e);
                        n.hasClass("select2-hidden-accessible") ? t.insertAfter(n.parent().find(".select2-container")[0]) : t.insertAfter(e),
                ...
                e.find("select").select2({ language: "vi" });
        }));
}
...
function toogleServicesNav() {
    if ($(".sticky-services").length) {
        var t = $(window).scrollTop();
        $(".sticky-services").offset().top,
            t > 690 && $(window).width() > 992
                ? ($(".sticky-services").addClass("fixed_nav"), $(".service-nav-placeholder").length || $(".sticky-services").after('<div class="service-nav-placeholder"></div>'))
                : ($(".sticky-services").removeClass("fixed_nav"), $(".service-nav-placeholder").remove()),
            $(document).height() - $(window).height() - $(window).scrollTop() < 50 ? $(".sticky-services").hide() : $(".sticky-services").show();
    }
}
function loadTopRightMenu(t) {
    var e = $(".top-right-menu"),
        n = $(".top-right-menu").attr("data-url");
    $.ajax({ url: n }).done(function (n) {
...
function showNotice(t, e, n) {
    var i = "<h3>" + n + "</h3>";
    $.jGrowl(i, { life: 4e3, header: e, speed: "slow", theme: t });
}
function homeCategoryBox(t) {
    var e = t.attr("data-url");
    t.addClass("loading"),
        $.ajax({ url: e }).done(function (e) {
            t.html(e), t.removeClass("loading"), t.find(".ajax-link").fancybox({ closeClickOutside: !0 });
        });
}
...
function autoSearch(t) {
    var e = t.find(".autosearch-input").val(),
        ...
        a = t.find("select").val();
    "" != e.trim()
        ? (t.find(".autosearch-result-box").length || (t.append('<div class="autosearch-result-box"><ul></ul></div>'), (i = t.find(".autosearch-result-box ul"))),
          autosearch_xhr && 4 != autosearch_xhr.readyState && autosearch_xhr.abort(),
          (autosearch_xhr = $.ajax({ url: n, data: { keyword: e, menu_id: a } }).done(function (t) {
              i.html(""),
                  i.parent().show(),
                  t.forEach(function (t) {
                      i.append(
                          '<li><a title="' +
                              ...
                              '" /><span class="title">' +
                              t.name +
                              '</span><span class="price sold_out_' +
                              t.is_sold_out +
                              " is_call_" +
                              t.is_call +
                              '">' +
                              ("<span style='font-family:sans-serif;'>0₫</span>" == t.price || t.is_sold_out || t.is_call ? "Liên Hệ: (028) 3984 7690" : t.price) +
                              '</span> <span class="old_price old_price_' +
                              t.old_price +
                              '"><span class="num">' +
                              t.old_price +
                              "</span> (giảm " +
...
function showHideMenuFixed() {
    ...
    $(window).scrollTop() > 165 && $(window).scrollTop() > t && $(window).width() > 992
        ? $(".vertical-wrapper").addClass("active-content-menu")
        : ($(".vertical-wrapper").removeClass("active-content-menu"), $(".vertical-wrapper").hasClass("transition") && $(".vertical-wrapper").removeClass("transition"));
}
function tabContent() {
    ...
    $(document).on("click", "a.link-method[data-method]", function (t) {
        if (!$(this).parents(".datalist-list-action").length) {
            ...
            var e = $(this).attr("data-method"),
                n = $(this).attr("href"),
                i = jQuery("<form>", { action: n, method: "POST", target: "_top" });
            i.append(jQuery("<input>", { name: "authenticity_token", value: AUTH_TOKEN, type: "hidden" })), i.append(jQuery("<input>", { name: "_method", value: e, type: "hidden" })), $(document.body).append(i), i.submit();
        }
    }),
        ...
        $(document).on("submit", "#new_newsletter", function (t) {
            ...
            "" != i.val().trim()
                ? $.ajax({ url: n, data: e.serialize() }).done(function (t) {
                      var e = "<h3>" + t.message + "</h3>";
                      $.jGrowl(e, { life: 4e3, header: "Thông báo", speed: "slow", theme: t.status }), i.val(""), i.removeClass("error");
                  })
                : (i.focus(), i.addClass("error"), $.jGrowl("<h3>Bạn phải nhập email đăng ký</h3>", { life: 4e3, header: "Thông báo", speed: "fast", theme: "warning" }));
        }),
        $(document).on("mousedown", ".ajax-link", function () {
            (void 0 !== $(this).attr("dont-close") && "true" == $(this).attr("dont-close")) || $.fancybox.close("all");
        }),
        $(".ajax-link").fancybox({ closeClickOutside: !0 }),
        $(document).on("submit", ".ajax-form", function (t) {
            ...
            var e = $(this),
                ...
                i = e.attr("method");
            e.find(".notice").hide(),
                $.ajax({ url: n, method: i, data: e.serialize(), dataType: "text" })
                    .done(function (t) {
                        location.reload();
                    })
                    .fail(function (t, n, i) {
        ...
        $(document).on("submit", ".register-form", function (t) {
            ...
            var e = $(this),
                ...
                i = e.attr("method");
            e.find(".notice").hide(),
                e.find("input[type=submit]").after('<div class="btn-loading text-center">Đang xử lý...</div>'),
                e.find("input[type=submit]").hide(),
                $.ajax({ url: n, method: i, data: e.serialize(), dataType: "text" }).done(function (t) {
        ...
        $(document).on("submit", ".add-cart-form", function (t) {
            ...
            var e = $(this),
                n = e.attr("action");
            e.addClass("loading"),
                $.ajax({ url: n, method: "POST", data: e.serialize() }).done(function (t) {
        ...
        $(document).on("submit", ".reset-password", function (t) {
            ...
            var e = $(this),
                ...
                i = e.attr("method");
            e.find(".notice").hide(),
                e.find("input[type=submit]").after('<div class="btn-loading text-right">Đang xử lý...</div>'),
                e.find("input[type=submit]").hide(),
                $.ajax({ url: n, method: i, data: e.serialize(), dataType: "text" }).done(function (t) {
                    $("<div>").html(t).find("form").length ? e.find(".form-content").html($("<div>").html(t).find(".form-content").html()) : e.find(".form-content").html(t);
                });
        ...
        $(function () {
            var t = $("<button/>", { type: "button", text: "x", id: "close-preview", style: "font-size: initial;" });
            t.attr("class", "close pull-right"),
                $(".image-preview").popover({ trigger: "manual", html: !0, title: "<strong>Xem trước</strong>" + $(t)[0].outerHTML, content: "There's no image", placement: "bottom" }),
                ...
                $(".image-preview-input input:file").change(function () {
                    var t = $("<img/>", { id: "dynamic", width: 250, height: 200 }),
                        e = this.files[0],
                        n = new FileReader();
                    (n.onload = function (n) {
                        $(".image-preview-input-title").text("Thay đổi"),
                            $(".image-preview-clear").show(),
                            $(".image-preview-filename").val(e.name),
                            t.attr("src", n.target.result),
                            $(".image-preview").attr("data-content", $(t)[0].outerHTML).popover("show");
                    }),
                        n.readAsDataURL(e);
                }),
                ...
                $(".rating_form").validate({ rules: { "rating[content]": { required: !0, minlength: 50 } } }),
                ...
                $(".layout-subpage .top-link li").hover(
                    function () {
        ...
        $(".autosearch select").select2({ minimumResultsForSearch: 10, dropdownAutoWidth: "true", language: "vi" }),
        ...
        $("select").each(function () {
            ...
            void 0 === t && (t = !1), $(this).select2({ minimumResultsForSearch: 10, dropdownAutoWidth: "true", language: "vi", placeholder: t });
        }),
        $(document).on("change", ".ajax-content-control", function () {
            var t = $($(this).attr("data-content-selector")),
                ...
                n = $(this).val();
            t.html(""),
                $.ajax({ url: e, data: { state_id: n } }).done(function (e) {
                    t.html(e), t.parents(".fancybox-inner").length || t.find("select").select2({ minimumResultsForSearch: 10, dropdownAutoWidth: "true", language: "vi" });
                });
        }),
        ...
        $(document).on("click", ".fancybox-slide--current", function (t) {
        ...
        $(document).on("change", ".category-filter-box input, .category-filter-box select, .category-filter-top input, .category-filter-top select", function () {
            ...
            var i = t.attr("id");
            void 0 === i && ((i = guid()), t.attr("id", i)),
                $.ajax({ url: e, data: $(".category-filter-box, #" + i).serialize() }).done(function (t) {
                    n.html($("<div>").html(t).find(".main-products-list").html()),
                        setTimeout(function () {
                        ...
                        n.find(".ajax-link").fancybox({ closeClickOutside: !0 });
                });
        }),
        $(document).on("click", ".box-pagination .pagination a", function (t) {
            ...
            var e = $(this).attr("href"),
                ...
                i = n.find("form"),
                a = i.attr("id");
            void 0 === a && ((a = guid()), i.attr("id", a)),
                n.prepend('<div class="category-loading"></div>'),
                n.find(".products-list").css("opacity", "0.6"),
                $(window).scrollTop($("#content").offset().top - 100),
                $.ajax({ url: e, data: $(".category-filter-box, #" + a).serialize() }).done(function (t) {
                    n.html($("<div>").html(t).find(".main-products-list").html()),
                        setTimeout(function () {
                            n.find(".product-image-container").addClass("lazy-loaded"), n.find(".product-image-container").removeClass("lazy"), n.find(".products-list").removeClass("list").addClass("grid");
                        }, 500),
        ...
        $(document).on("click", "[link-method]", function (t) {
            ...
            if (confirm(i)) {
                var a = jQuery("<form>", { action: n, method: "POST", target: "_top" });
                a.append(jQuery("<input>", { name: "authenticity_token", value: AUTH_TOKEN, type: "hidden" })), a.append(jQuery("<input>", { name: "_method", value: e, type: "hidden" })), $(document.body).append(a), a.submit();
            }
        }),
        ...
        $(document).on("click", ".quick-view.btn_compare", function (t) {
            ...
            $.ajax({ url: e, method: "POST", data: { authenticity_token: AUTH_TOKEN, product_id: n } }).done(function (t) {
        ...
        $(document).on("submit", ".online_search_form", function (t) {
            ...
            "" !== a.trim() &&
                (o.html('<div class="category-loading"></div>'),
                online_search_xhr && 4 != online_search_xhr.readyState && online_search_xhr.abort(),
                (online_search_xhr = $.ajax({ url: n, method: i, data: e.serialize() }).done(function (t) {
                    o.html(t),
                        setTimeout(function () {
        ...
        $(document).on("click", ".service_box_list .pagination a", function (t) {
            ...
            n.html('<div class="category-loading"></div>'),
                online_search_xhr && 4 != online_search_xhr.readyState && online_search_xhr.abort(),
                (online_search_xhr = $.ajax({ url: e, method: "GET" }).done(function (t) {
                    n.html(t),
                        setTimeout(function () {
                            n.find(".product-image-container").addClass("lazy-loaded"), n.find(".product-image-container").removeClass("lazy"), n.find(".products-list").removeClass("list").addClass("grid");
                        }, 500),
        ...
        $(document).on("submit", ".quick-view-form", function (t) {
            ...
            var e = $(this),
                ...
                i = e.attr("method");
            return (
                $.ajax({ url: n, method: i, data: e.serialize() }).error(function () {
        ...
        $(document).on("click", ".cart-item-delete", function (t) {
            ...
            var e = $(this).attr("href");
            return (
                $.ajax({ url: e }).done(function () {
                    var t = $(".shopping-cart").attr("data-url");
                    $.ajax({ url: t }).done(function (t) {
                        showNotice("success", "Thành công", "Giỏ hàng đã được cập nhật"),
                            $(".shopping-cart .table-responsive.form-group").html($("<div>").html(t).find(".shopping-cart .table-responsive.form-group").html()),
                            $("<div>").html(t).find(".shopping-cart .table-responsive.form-group").length || $.fancybox.close(),
                            loadTopCart();
                    });
                }),
                !1
...
$(".megamenu").hover(
    ...
    $(document).ready(function () {
        ...
        $(window).scroll(function () {
            $(window).width() >= 1010 && $(window).scrollTop() > $("#header").height() + $("#yt_header_right").height()
                ? $("#btn-menu-fixed-title").hasClass("active-btn")
